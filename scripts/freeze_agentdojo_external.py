...
def freeze_slice(
    ...
    *,
    ...
    if config["agentdojo_commit"] != "a75aba7631d3ca5fb7ab938965c97ead2f9ff84b" or config["agentdojo_tag"] != "v0.1.35":
        raise ValueError("external config does not pin the approved AgentDojo revision")
...
def verify_slice(output_dir: Path) -> dict[str, Any]:
    output_dir = output_dir.resolve()
    ...
    development = [AgentDojoPair.model_validate(json.loads(line)) for line in (output_dir / "selected_pairs.jsonl").read_text(encoding="utf-8").splitlines() if line][:2]
    formal = selected[2:]
...
def main(argv: list[str] | None = None) -> int:
    ...
    mode.add_argument("--verify", type=Path, metavar="OUTPUT_DIR")
    parser.add_argument("--output-dir", type=Path)
    args = parser.parse_args(argv)
    if args.verify is not None:
        print(json.dumps(verify_slice(args.verify), sort_keys=True))
        return 0
