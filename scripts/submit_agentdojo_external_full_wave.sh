...
for suite in workspace travel banking slack; do
  ...
  if [[ "${PHASE}" == development ]]; then
    AGENTDOJO_FROZEN_ROOT="${FROZEN_ROOT}" AGENTDOJO_ARTIFACT_ROOT="${suite_artifacts}" AGENTDOJO_SUITE="${suite}" VLLM_BASE_PORT="${suite_base_port}" \
      sbatch "${ROOT}/scripts/run_agentdojo_external.slurm" development 0 "$(awk 'NF{n++} END{print n+0}' "${plan}")" attempt-0001 "${suite}"
  else
    AGENTDOJO_FROZEN_ROOT="${FROZEN_ROOT}/${suite}" AGENTDOJO_ARTIFACT_ROOT="${suite_artifacts}" VLLM_BASE_PORT="${suite_base_port}" \
